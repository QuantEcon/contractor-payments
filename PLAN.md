...
- [ ] **Repo settings to apply on creation** (every new contractor repo gets the same baseline):
  ...
  - **Branch protection on `main` via rulesets (not legacy branch protection).** Required state: PR required, **1 approving review required**, require Code Owners, no force-push, no direct push. The admin still reviews their own approval but the "Approve" click is a useful safety gate before the merge triggers email + ledger + email-to-PSL once `testing_mode` flips. Implementation note: use **rulesets** rather than legacy branch protection so we can grant the `github-actions[bot]` actor a bypass for the `[skip ci]` ledger pushes from `process-approved.yml`. Legacy branch protection only offers `enforce_admins: true/false` (all-or-nothing) — `true` blocks the bot's push and breaks the workflow, `false` lets admins merge without approving and defeats the gate. Rulesets `bypass_actors` solve this cleanly: gate is hard for humans, bot bypasses.
