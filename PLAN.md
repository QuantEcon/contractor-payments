...
- [x] **Repo settings to apply on creation** (every new contractor repo gets the same baseline):
  ...
  - **Branch protection on `main` via an org-level ruleset (not per-repo legacy branch protection).** Required state: PR required, **1 approving review required**, require Code Owners, no force-push, no direct push. The admin still reviews their own approval but the "Approve" click is a useful safety gate before the merge triggers email + ledger + email-to-PSL once `testing_mode` flips. **Implementation note (important — proven the hard way during Phase 2.5 testing):**
    ...
    - **Repo-level rulesets can't grant bypass to the `github-actions` system app** (rejects with "Actor GitHub Actions integration must be part of the ruleset source or owner organization"). The bypass only works when the ruleset is created at the **org level** by an org admin, targeting repos by name pattern (e.g. `contractor-*`).
    - **Phase 3b checklist item for QuantEcon org admin (one-time, not per-repo):** create an org-level ruleset on `QuantEcon` org named e.g. `contractor-repos-main-protection`, targeting repos matching `contractor-*` and the default branch, with:
      - Rule: pull request required, 1 approving review, require code owner review.
