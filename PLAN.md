...
- [ ] **GitHub org-level secrets** — `SMTP_HOST` ✅, `SMTP_PORT` ✅ already set. **Still pending:** `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`. Scoped at the org (Private repositories) so every contractor repo's reusable-workflow run can read them. See §10 (SMTP credentials open item) and `notes/EMAIL_SETUP.md` for the runbook.
